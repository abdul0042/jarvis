  ...
  try {
    ...
    if (state === 'sheets') {
      ...
      return res.send(`
        ...
        <body style="background:#000;color:#00ff41;font-family:monospace;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;flex-direction:column;gap:12px">
          ...
          <script>
            try {
              window.opener.postMessage(${JSON.stringify({ type: 'sheets-tokens', tokens })}, '${process.env.CLIENT_URL || 'http://localhost:5173'}');
            } catch(e) {}
    ...
    res.send(`
      ...
      <body style="background:#000;color:#00ff41;font-family:monospace;display:flex;align-items:center;justify-content:center;height:100vh;margin:0;flex-direction:column;gap:12px">
        ...
        <script>
          try {
            window.opener.postMessage(${JSON.stringify({ type: 'gmail-tokens', tokens })}, '${process.env.CLIENT_URL || 'http://localhost:5173'}');
          } catch(e) {}
          setTimeout(() => window.close(), 1000);
        </script>
      </body>
